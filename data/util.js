...
App.util = {
  ...
  /**
   * Small helper for addEventListener with tolerant inputs.
   * - target can be element or selector/id string
   * - no-op if target not found
   */
  on(target, eventName, handler, options) {
    try {
      const el = (typeof target === 'string') ? App.util.$(target) : target;
      if (!el || !el.addEventListener) return null;
      el.addEventListener(eventName, handler, options);
      return () => {
        try { el.removeEventListener(eventName, handler, options); } catch(_) {}
      };
    } catch(_) {
      return null;
    }
