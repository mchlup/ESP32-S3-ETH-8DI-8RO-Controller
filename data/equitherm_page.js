...
(function(){
  ...
  function drawGraph(canvas, model){
    ...
    const eqSafe = (model && model.eq) ? model.eq : {};
    let yMin = num(eqSafe.minFlow, 25);
    let yMax = num(eqSafe.maxFlow, 75);
    ...
    function curve(slope, shift, color){
      ...
      for(let t=tMin; t<=tMax; t+=1){
        let sp = eqTarget(t, slope, shift) + num(eqSafe.curveOffsetC,0);
    ...
    curve(num(eqSafe.slopeDay,1.6), num(eqSafe.shiftDay,0), '#2e7dff');
    curve(num(eqSafe.slopeNight,1.6), num(eqSafe.shiftNight,0), '#ff9800');

    // current point
    const tout = num(model.outdoorC, 0);
    const isNight = !!model.night;
    const slope = isNight ? num(eqSafe.slopeNight,1.6) : num(eqSafe.slopeDay,1.6);
    const shift = isNight ? num(eqSafe.shiftNight,0) : num(eqSafe.shiftDay,0);
    let sp = eqTarget(tout, slope, shift) + num(eqSafe.curveOffsetC,0);
    sp = clamp(sp, yMin, yMax);
    ...
    root.innerHTML = `
      <div class="pageHead">
        ...
        <div class="pageActions">
          <button class="btn" id="eqLoadBtn">Načíst</button>
          <button class="btn primary" id="eqSaveBtn">Uložit</button>
        </div>
      </div>
      ...
      <div class="grid12">
        ...
        <div class="card col6">
          ...
          <div class="kv" style="grid-template-columns: 200px 1fr;">
            <div class="k">Dolní mez teploty TOP</div><div class="v"><input class="inp w140" id="minFlow" type="number" min="0" max="80" step="0.5" /> °C</div>
            <div class="k">Horní mez teploty TOP</div><div class="v"><input class="inp w140" id="maxFlow" type="number" min="0" max="80" step="0.5" /> °C</div>
            <div class="k">Korekce křivky (curveOffsetC)</div><div class="v"><input class="inp w140" id="curveOffsetC" type="number" min="-10" max="10" step="0.1" /> °C</div>
            <div class="k">Max. teplota kotle (maxBoilerInC)</div><div class="v"><input class="inp w140" id="maxBoilerInC" type="number" min="20" max="80" step="0.5" /> °C</div>
            <div class="k">Fallback venkovní teplota</div><div class="v"><input class="inp w140" id="fallbackOutdoorC" type="number" min="-40" max="60" step="0.5" /> °C</div>
          </div>
        ...
        <div class="card col6">
          ...
          <div class="kv" style="grid-template-columns: 220px 1fr;">
            <div class="k">Hystereze (deadbandC)</div><div class="v"><input class="inp w140" id="deadbandC" type="number" min="0" max="5" step="0.1" /> °C</div>
            <div class="k">Krok (stepPct, 0 = auto)</div><div class="v"><input class="inp w140" id="stepPct" type="number" min="0" max="25" step="1" /> %</div>
            <div class="k">Perioda (periodMs)</div><div class="v"><input class="inp w140" id="periodMs" type="number" min="500" max="600000" step="100" /> ms</div>
            <div class="k">Min. otevření (minPct)</div><div class="v"><input class="inp w140" id="minPct" type="number" min="0" max="100" step="1" /> %</div>
            <div class="k">Max. otevření den</div><div class="v"><input class="inp w140" id="maxPctDay" type="number" min="0" max="100" step="1" /> %</div>
            <div class="k">Max. otevření noc</div><div class="v"><input class="inp w140" id="maxPctNight" type="number" min="0" max="100" step="1" /> %</div>
          </div>
        </div>
        ...
        <div class="card col6">
          ...
          <div class="kv" style="grid-template-columns: 250px 1fr;">
            <div class="k">Doba přesunu A↔B (travelTimeS)</div><div class="v"><input class="inp w140" id="travelTimeS" type="number" min="1" max="600" step="0.1" /> s</div>
            <div class="k">Délka pulzu (pulseTimeS)</div><div class="v"><input class="inp w140" id="pulseTimeS" type="number" min="0.05" max="10" step="0.05" /> s</div>
            <div class="k">Ochranná prodleva (guardTimeS)</div><div class="v"><input class="inp w140" id="guardTimeS" type="number" min="0" max="10" step="0.05" /> s</div>
            <div class="k">Min. přepnutí směru (minSwitchS)</div><div class="v"><input class="inp w140" id="minSwitchS" type="number" min="0" max="600" step="0.1" /> s</div>
            ...
            <div class="k">Výchozí poloha (defaultPos)</div><div class="v">
              <select id="defaultPos" class="sel w140">
          ...
          <div class="btnRow" style="margin-top:10px;display:flex;gap:8px;align-items:center;flex-wrap:wrap">
            ...
            <button class="btn sm" id="btnValveCalibrate">Kalibrace HOME</button>
        ...
        <div class="card col6">
          ...
          <div class="kv" style="grid-template-columns: 260px 1fr;">
            ...
            <div class="k">Timeout (noFlowTimeoutMs)</div><div class="v"><input class="inp w140" id="noFlowTimeoutMs" type="number" min="10000" max="3600000" step="1000" /> ms</div>
            <div class="k">Test perioda (noFlowTestPeriodMs)</div><div class="v"><input class="inp w140" id="noFlowTestPeriodMs" type="number" min="10000" max="3600000" step="1000" /> ms</div>
          </div>
        </div>
  ...
  function bind(){
    ...
    const btnCal = document.getElementById('btnValveCalibrate');
    ...
    if(btnCal) btnCal.addEventListener('click', async ()=>{
      const master = model.valveMaster || 1;
      try{
        await App.api.postText(`/api/equitherm/calibrate?master=${master}`, '', 'text/plain');
        const el = document.getElementById('eqValveTestInfo');
        if(el) el.textContent = `master R${master} • calibrate(home)`;
        App.util && App.util.toast && App.util.toast('Kalibrace spuštěna', 'ok');
      }catch(e){
        console.error(e);
        App.util && App.util.toast && App.util.toast('Kalibrace selhala', 'err');
      }
    });
  }
