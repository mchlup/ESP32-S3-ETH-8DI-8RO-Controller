...
(() => {
  ...
  try { return JSON.stringify(norm(obj)); }
  catch (_) { return JSON.stringify(obj); }
};
...
cfg.iofunc = (cfg.iofunc && typeof cfg.iofunc === "object") ? cfg.iofunc : {};
  ...
  const wireEvents = () => {
    const bindOnce = (el, type, key, handler, opts) => {
      if (!el) return;
      const k = `wired${key}`;
    ...
    // nav
    $$(".navItem").forEach(b => bindOnce(b, "click", "NavItem", () => showPage(String(b.dataset.page||"dashboard"))));
    bindOnce($("#btnNav"), "click", "BtnNav", () => {
      $("#sidebar")?.classList.toggle("open");
    });
    bindOnce($("#btnCollapse"), "click", "BtnCollapse", () => {
    ...
    applyTheme(current);
    bindOnce($("#btnTheme"), "click", "BtnTheme", () => {
    ...
    bindOnce($("#btnReloadEquitherm"), "click", "ReloadEquitherm", reloadIfClean);
    bindOnce($("#btnReloadAku"), "click", "ReloadAku", reloadIfClean);
    bindOnce($("#btnReloadTuv"), "click", "ReloadTuv", reloadIfClean);
    bindOnce($("#btnReloadRecirc"), "click", "ReloadRecirc", reloadIfClean);
    bindOnce($("#btnReloadAkuHeater"), "click", "ReloadAkuHeater", reloadIfClean);

    // relay toggles (legacy dashboard)
    bindOnce($("#relayGrid"), "click", "RelayGrid", async (e) => {
    ...
    bindOnce($("#modesGrid"), "click", "ModesGrid", (e) => {
    ...
    // apply control/mode
    bindOnce($("#btnApplyMode"), "click", "ApplyMode", async () => {
      try{
    ...
    bindOnce($("#selControlMode"), "change", "SelControlMode", () => { lockUI("control", 2500); scheduleAutoApply(); });
    bindOnce($("#selSystemMode"), "change", "SelSystemMode", () => { lockUI("system", 2500); scheduleAutoApply(); });

    bindOnce($("#btnAutoRecompute"), "click", "AutoRecompute", async () => {
      try{ await autoRecompute(); toast("AUTO přepočteno"); await loadStatus(); }
      catch(err){ toast("Chyba: " + err.message, "bad"); }
    });

    // save config (forms)
    bindOnce($("#btnSaveInputs"), "click", "SaveInputs", () => saveInputsFromForm().catch(e=>toast(e.message,"bad")));
    bindOnce($("#btnSaveRelays"), "click", "SaveRelays", () => saveRelaysFromForm().catch(e=>toast(e.message,"bad")));
    bindOnce($("#btnSaveModes"), "click", "SaveModes", () => saveModesFromForm().catch(e=>toast(e.message,"bad")));

    // cfg json
    bindOnce($("#cfgJson"), "input", "CfgJson", () => {
      if (state.ui?.dirty) state.ui.dirty.cfgJson = true;
    });

    bindOnce($("#btnFmtCfg"), "click", "FmtCfg", () => {
      const obj = safeJson($("#cfgJson").value);
      if (!obj) return toast("Neplatný JSON", "bad");
      $("#cfgJson").value = prettyJson(obj);
      if (state.ui?.dirty) state.ui.dirty.cfgJson = true;
    });
    bindOnce($("#btnSaveCfgJson"), "click", "SaveCfgJson", () => saveConfigJsonFromEditor().catch(e=>toast(e.message,"bad")));

    // mqtt
    bindOnce($("#btnSaveMqtt"), "click", "SaveMqtt", () => saveMqtt().catch(e=>toast(e.message,"bad")));
    bindOnce($("#btnMqttDiscovery"), "click", "MqttDiscovery", () => mqttDiscovery().catch(e=>toast(e.message,"bad")));

    // ble
    bindOnce($("#btnSaveBle"), "click", "SaveBle", () => saveBle().catch(e=>toast(e.message,"bad")));
    bindOnce($("#btnStartPair"), "click", "StartPair", () => startPair().catch(e=>toast(e.message,"bad")));
    bindOnce($("#btnStopPair"), "click", "StopPair", () => stopPair().catch(e=>toast(e.message,"bad")));
    bindOnce($("#btnMeteoRetry"), "click", "MeteoRetry", async () => {
      try {
        await apiPostJson("/api/ble/meteo/retry", {});
        toast("Meteo: retry", "good");
        await loadBle({ force: true });
      } catch (e) {
        toast(e.message || "Chyba", "bad");
      }
    });

    // files
    bindOnce($("#btnRefreshFiles"), "click", "RefreshFiles", () => loadFiles().catch(e=>toast(e.message,"bad")));
    bindOnce($("#uploadForm"), "submit", "UploadForm", async (e) => {
      e.preventDefault();
    ...
    // reboot
    bindOnce($("#btnReboot"), "click", "Reboot", async () => {
  ...
  const init = () => {
    ...
    updateDirtyIndicators();
    wireEvents();
    // Router mounts pages dynamically -> bind handlers after each page mount
    if (!window.__heatui_legacyWired) {
      window.__heatui_legacyWired = true;
      window.addEventListener("app:pageMounted", () => {
        try { wireEvents(); } catch (_) {}
      });
    }
  };
