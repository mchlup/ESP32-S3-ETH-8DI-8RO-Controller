...
(function(){
    ...
    hostEl.innerHTML = `
      <div class="pageHead">
        ...
        <div class="pageActions">
          ...
          <button class="btn primary" id="dhwSave">Uložit</button>
        </div>
      </div>
  ...
  function updateStatusUI(fast){
    ...
    const elDemand = $('#dhwStDemand', hostEl);

    const setTxt = (sel, v) => { const el = $(sel, hostEl); if(el) el.textContent = v; };
    const setVal = (sel, v, cls) => { const el = $(sel, hostEl); if(!el) return; el.textContent = v; if(cls!=null) el.className = cls; };

    if(elDemand) elDemand.textContent = (demand==null) ? '—' : (demand ? 'ACTIVE' : 'bez požadavku');
    ...
    setVal('#dhwStEnabled', tuv.enabled ? 'true' : 'false', 'v ' + (tuv.enabled ? 'ok' : ''));
    setVal('#dhwStActive', tuv.active ? 'true' : 'false', 'v ' + (tuv.active ? 'ok' : ''));
    setVal('#dhwStBoiler', tuv.boilerRelayOn ? 'ON' : 'OFF', 'v ' + (tuv.boilerRelayOn ? 'ok' : ''));
    setTxt('#dhwStReason', tuv.reason || '—');
  }
  ...
  App.pages.dhw = {
    async mount(host){
      ...
      statusTimer = setInterval(()=>{
        if(!hostEl || !hostEl.isConnected){
          try{ clearInterval(statusTimer); }catch(_){}
          statusTimer = null;
          return;
        }
        reloadStatus();
    ...
    unmount(){
      hostEl = null;
      if(statusTimer){ try{ clearInterval(statusTimer); }catch(_){} statusTimer=null; }
    }
  };
