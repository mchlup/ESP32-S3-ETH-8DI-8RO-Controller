...
(() => {
  ...
  const render = () => {
    ...
    for (let i = 0; i < RELAY_COUNT; i++) {
      ...
      const partner = (i + 2 <= RELAY_COUNT) ? (i + 2) : Number(o.params?.partnerRelay || 0);
      items.push({ idx: i, partner });
    }

    if (!items.length) {
      host.innerHTML = `<div class="pad muted">Nenalezen žádný trojcestný ventil. Nastav ho v záložce „Funkce I/O“ jako „Trojcestný ventil 2‑bod“ (master) – použije DOx + DO(x+1).</div>`;
      return;
    }
    ...
    items.forEach(({ idx, partner }) => {
      ...
      row.dataset.partner = String(partner || 0);
  ...
  const bind = () => {
    ...
    host.addEventListener("click", async (ev) => {
      ...
      if (!partner) {
        toast("Chybí relé B (partner). Trojcestný ventil potřebuje DOx + DO(x+1).", "bad");
        return;
      }
