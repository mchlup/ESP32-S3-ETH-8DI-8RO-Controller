...
async function saveConfig(){
  ...
  const inPin = Math.round(N($('#ot_inPin',H)?.value));
  const outPin = Math.round(N($('#ot_outPin',H)?.value));
  ...
  if(Number.isFinite(inPin)) ot.inPin=inPin;
  if(Number.isFinite(outPin)) ot.outPin=outPin;
...
function render(){
  ...
  '<div class="grid2">'+
    ...
    '<div class="card">'+
      ...
      '<div class="card__body">'+
        ...
        '<div class="grid2" style="margin-top:12px">'+
          '<div class="field"><div class="label">OT inPin (future)</div><input id="ot_inPin" type="number" step="1" value="'+E(String(c.inPin??-1))+'"></div>'+
          '<div class="field"><div class="label">OT outPin (future)</div><input id="ot_outPin" type="number" step="1" value="'+E(String(c.outPin??-1))+'"></div>'+
        '</div>'+

        '<div class="row" style="gap:8px;margin-top:12px;flex-wrap:wrap">'+
          '<button class="btn ok" id="ot_save">Uložit</button>'+
          '<button class="btn" id="ot_reload">Načíst ze zařízení</button>'+
        '</div>'+
  ...
  $('#ot_save',H).onclick=saveConfig;
  $('#ot_reload',H).onclick=async()=>{
    const r=await fetchFull();
    if(r){ST=r.status;CFG=r.config||CFG;render();}
  };
}
