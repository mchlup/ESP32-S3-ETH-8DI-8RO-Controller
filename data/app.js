    ...
    function getUiSample(){
      ...
      const mixSupply = firstFinite(state?.eqFast?.ma, ch, last?.mixSupply);
      const mixReturn = firstFinite(state?.eqFast?.mb, temps.returnDallasC, temps.return, temps.returnTempC, state?.ot?.returnTempC, last?.mixReturn);
      const afterMix = firstFinite(state?.eqFast?.mf, getAfterMixTempFromTemps(temps), last?.mixAfter, state?.accu?.after, ch);
      return {
        now,
...
state.th = {
  ...
  mixingValve:{ a:"tank_mid", b:"return_dallas", ab:"opentherm_ch" },
...
function normalizeMixingValveSources(raw){
  ...
  return {
    a: String(src.a || src.sourceA || "tank_mid").trim() || "tank_mid",
    b: String(src.b || src.sourceB || "return_dallas").trim() || "return_dallas",
    ab: String(src.ab || src.sourceAB || "opentherm_ch").trim() || "opentherm_ch",
  };
}
...
function mixPortFastValue(port){
  const eq = state.eqFast || {};
  const key = String(port || "").toLowerCase();
  if(key === "a") return numOrNaN(eq.ma);
  if(key === "b") return numOrNaN(eq.mb);
  if(key === "ab") return numOrNaN(eq.mf);
  return NaN;
}
...
// ----- Shared UI helpers
    ...
    // Update endpoints/fields to match your firmware.
      ...
      async mixCmdImmediate(action, payload={}){
        ...
        try{
          ...
          if(act === "pulse_a") return await api.eqCmd({ mixPulse:"a", pulseMs });
          if(act === "pulse_b") return await api.eqCmd({ mixPulse:"b", pulseMs });
          if(act === "end_a") return await api.eqCmd({ mixMove:"a_end" });
          if(act === "end_b") return await api.eqCmd({ mixMove:"b_end" });
          if(act === "stop") return await api.eqCmd({ mixPulse:"stop" });
          throw new Error("unsupported_mix_action");
        }
      },
    ...
    function mixStateMeta(rawState){
      ...
      const map = {
        ...
        support_target_hold: ["cíl podpory dosažen • ventil setrvává", "good"],
        support_target_settling: ["cíl podpory v pásmu • probíhá stabilizační interval", "warn"],
        support_target_confirming: ["cíl podpory musí zůstat v pásmu po celý min. interval", "warn"],
        support_target_unverified_hold: ["teplota je v cílovém pásmu, ale pohyb ventilu nebyl potvrzen – bez ověřovacího pulsu", "warn"],
        support_wait_settle_open: ["čeká na ustálení teploty AB před dalším pulsem do A", "warn"],
        support_wait_settle_close: ["čeká na ustálení teploty AB před dalším pulsem do B", "warn"],
        support_target_return_a: ["cíl podpory dosažen • návrat do A", "good"],
        support_target_return_a_pending: ["čeká na návrat do A", "warn"],
        support_target_at_a: ["cíl podpory dosažen • poloha A", "good"],
        ...
        fault_relay_write: ["porucha: stav R1/R2 nebyl potvrzen expandérem", "bad"],
        blocked_summer: ["blokováno letním režimem", "warn"],
        blocked_summer_closing: ["letní režim • zavírání ventilu", "warn"],
        blocked_dhw_positioning: ["priorita TUV • přesun ventilu", "warn"],
        protect_boiler: ["ochrana kotle • otevírá", "bad"],
        protect_boiler_dhw: ["ochrana kotle během priority TUV • havarijně otevírá", "bad"],
        protect_boiler_pending: ["ochrana kotle • čeká na otevření", "bad"],
        protect_floor_max: ["ochrana podlahy • zavírá", "bad"],
        protect_floor_max_pending: ["ochrana podlahy • čeká na zavření", "bad"],
        protect_return_min: ["ochrana zpátečky • zavírá", "warn"],
        protect_return_min_pending: ["ochrana zpátečky • čeká na zavření", "warn"],
        return_protect_open: ["ochrana zpátečky • otevírá", "good"],
        return_protect_close: ["ochrana zpátečky • zavírá", "warn"],
        return_protect_band: ["ochrana zpátečky • v pásmu", "good"],
        return_feedback_missing: ["ochrana zpátečky • chybí čidlo", "bad"],
        closed_weekly: ["mimo týdenní program • zavřeno", "warn"],
        closed_outside: ["venkovní teplota • zavřeno", "warn"],
        ...
        manual_hold: ["ochranná prodleva po ručním zásahu", "warn"],
      };
      const hit = map[s];
    ...
    function mixDirectionPresentation(rawState){
      const raw = String(rawState || "idle").toLowerCase();
      if(/open|manual_a|calibrating_a|a_end|support_target_return_a/.test(raw)) return { text:"OTVÍRÁ (A → AB)", kind:"open" };
      if(/close|manual_b|calibrating_b|b_end/.test(raw)) return { text:"ZAVÍRÁ (AB → B)", kind:"close" };
      if(/fault/.test(raw)) return { text:"PORUCHA POHONU", kind:"fault" };
      if(/blocked/.test(raw)) return { text:"POHYB BLOKOVÁN", kind:"blocked" };
      return { text:"VENTIL STOJÍ", kind:"idle" };
    }
    ...
    function renderMixingDashboard(values){
      ...
      const rawState = String(mix.state || state.eqFast?.mix?.state || "idle");
      const direction = mixDirectionPresentation(rawState);
      const calibration = String(mix.calibration || state.eqStatus?.mix?.calibration || "unknown");
      const travelMs = Number(document.getElementById("hMixTravelMs")?.value || 6000);
      ["mixUiPosition"].forEach(id => setText(`#${id}`, `${Math.round(pct)} %`));
      setText("#mixUiDirection", direction.text);
      setText("#mixUiSchematicState", `poloha ${Math.round(pct)} %`);
      ...
      const temps = [
        ["mixUiSupplyTemp", values?.supplyC], ["mixUiSupplyTemp2", values?.supplyC],
        ["mixUiReturnTemp", values?.returnC], ["mixUiReturnTemp2", values?.returnC],
        ["mixUiOutputTemp", values?.outputC], ["mixUiOutputTemp2", values?.outputC],
        ["mixUiTargetTemp", values?.targetC],
      ];
      temps.forEach(([id,val]) => setText(`#${id}`, fmtNum(Number(val), 1)));
      setText("#mixCalTravelTime", `${(travelMs/1000).toLocaleString("cs-CZ", {minimumFractionDigits:1, maximumFractionDigits:1})} s`);
      setText("#mixCalState", mixCalibrationLabel(calibration));
      const calStatus = document.getElementById("mixCalStatus");
      const trusted = !!(mix.positionTrusted ?? state.eqStatus?.mix?.positionTrusted);
      if(calStatus){
        ...
        if(title) title.textContent = trusted ? "Kalibrace je platná" : "Kalibrace vyžaduje kontrolu";
        if(note) note.textContent = trusted ? "Poloha je odvozena od potvrzených krajních bodů." : "Proveďte přejezd do krajních poloh a oba body potvrďte.";
      }
      setText("#mixCalChartLabel", `${direction.text.toLowerCase()} • ${Math.round(pct)} %`);
      drawMixCalibrationChart(document.getElementById("mixCalChart"), pct, travelMs, direction.kind);
    }
    ...
    function renderMixCalibrationInfo(){
      ...
      const supportActive = !!(mix.supportActive ?? state.eqFast?.sa ?? state.eqFast?.mix?.sa);
      const supportReached = !!(mix.supportTargetReached ?? state.eqFast?.sr ?? state.eqFast?.mix?.sr);
      const supportAction = String(mix.supportAction || state.eqFast?.mix?.act || "");
      if(supportActive) parts.push("podpora AKU aktivní");
      if(supportReached) parts.push("cíl podpory dosažen");
      if(supportAction === "hold") parts.push("po cíli: setrvání");
      else if(supportAction === "return_a") parts.push("po cíli: návrat A");
      ...
      const posBar = document.getElementById("hMixPositionBar");
      const posLabel = document.getElementById("hMixPositionLabel");
      const posProgress = document.getElementById("hMixPositionProgress");
      const posTrust = document.getElementById("hMixPositionTrust");
      const trusted = !!(mix.positionTrusted ?? state.eqStatus?.mix?.positionTrusted);
      if(Number.isFinite(pct)){
        const safePct = clamp(Number(pct), 0, 100);
        if(posBar) posBar.style.width = `${safePct}%`;
        if(posLabel) posLabel.textContent = `${fmtNum(safePct, 0)} %`;
        if(posProgress) posProgress.setAttribute("aria-valuenow", String(Math.round(safePct)));
      }else{
        if(posBar) posBar.style.width = "0%";
        if(posLabel) posLabel.textContent = "-- %";
      }
      if(posTrust) posTrust.textContent = trusted ? "Poloha je odvozena od poslední kalibrace a časů pohybu." : "Poloha je pouze orientační; proveďte přejezd do krajní polohy nebo potvrďte aktuální krajní polohu.";
...
async function otRwWrite(){
  try{
    ...
    function syncTechI3UiVisibility(){
      ...
      curveGroup?.querySelectorAll("input,select").forEach(el => { el.disabled = returnProfile; });
      const curvePoints = document.getElementById("mixI3CurvePoints");
      if(curvePoints) curvePoints.hidden = returnProfile || String(document.getElementById("hMixCurveMode")?.value || "linear2") !== "tech_i3_4point";

      const hint = document.getElementById("mixI3ProfileHint");
      if(hint){
    ...
    function applyEqConfigToForm(cfg, options={}){
      ...
      if(state.th){
        state.th.mixingValve = normalizeMixingValveSources({
          a: mx.sourceA ?? state.th.mixingValve?.a,
          b: mx.sourceB ?? state.th.mixingValve?.b,
          ab: mx.sourceAB ?? state.th.mixingValve?.ab,
        });
      }
      ...
      if(document.getElementById("hMixControlMode")) document.getElementById("hMixControlMode").value = String(mx.controlMode || "adaptive");
      if(document.getElementById("hMixValveType")) document.getElementById("hMixValveType").value = String(mx.valveType || "ch");
      setInputNumber("hMixControlIntervalMs", mx.controlIntervalMs, 0);
      ...
      setInputNumber("hMixReturnProtectionMinC", mx?.returnProtection?.minC, 1);
      setInputBool("hMixFloorProtectionEnabled", mx?.floorProtection?.enabled);
      setInputNumber("hMixFloorProtectionMaxC", mx?.floorProtection?.maxC, 1);
      setInputBool("hMixFloorSummerEnabled", mx?.floorProtection?.summerEnabled);
      setInputBool("hMixOutsideCloseEnabled", mx?.outsideClose?.enabled);
      ...
      const w4n = Array.isArray(mx?.weather4?.night) ? mx.weather4.night : [];
      [["hMixDayM20",w4d[0]],["hMixDayM10",w4d[1]],["hMixDay0",w4d[2]],["hMixDayP10",w4d[3]],
       ["hMixNightM20",w4n[0]],["hMixNightM10",w4n[1]],["hMixNight0",w4n[2]],["hMixNightP10",w4n[3]]]
        .forEach(([id,value]) => setInputNumber(id, value, 1));
...
async function serviceIoCall(payload){
  ...
  setBadge("#serviceIoBadge", "good", "service: odesláno");
    ...
    function applyEqStatus(status){
      ...
      if(Number.isFinite(Number(temps.mixAC))) state.eqFast.ma = Number(temps.mixAC);
      if(Number.isFinite(Number(temps.mixBC))) state.eqFast.mb = Number(temps.mixBC);
      if(Number.isFinite(Number(temps.mixABC))) state.eqFast.mf = Number(temps.mixABC);
      state.eqFast.mix = state.eqFast.mix || {};
...
// Accumulator tank + mixing valve
      ...
      const supportRuntime = state.eqStatus?.mix?.supportActive ?? state.eqFast?.sa ?? state.eqFast?.mix?.sa;
      const helping = supportRuntime != null
        ? !!supportRuntime
...
async function refresh(forceToast=false){
  ...
  try{
    ...
    renderMixCalibrationInfo();
      ...
      ["hDaySlope","hDayShift","hNightSlope","hNightShift","hMin","hMax","hBoilerMax","hTarget","eqSet","hMixDeadband","hMixTargetOffsetC","hMixTargetReachedAction","hMixPulseMs","hMixMinIntervalMs","hMixTravelMs","hMixEnabled","hMixOpenRelay","hMixCloseRelay","hEqModeCfg","hUseIn1NightOverride","hSummerModeEnabled","hSummerOffAboveC","hSummerOnBelowC","hDriveNightRelay","hNightRelay","hNightRelayOnWhenNight","hBoilerAssistEnabled","hBoilerAssistForceChEnable"].forEach(id => {
        const el = document.getElementById(id);
        if(!el) return;
...
updatePlannerStateBadges();
      ...
      const waitForButtonIdle = (button, timeoutMs=20000) => new Promise((resolve) => {
        const started = Date.now();
        const timer = setInterval(() => {
          if(!button || button.dataset.busy !== "1" || Date.now()-started > timeoutMs){ clearInterval(timer); resolve(); }
        }, 80);
      });
      ...
      if(mixSaveBtn){
        mixSaveBtn.addEventListener("click", () => withButtonBusy(mixSaveBtn, "Ukládám…", async () => {
          const heatingSaveBtn = document.getElementById("hApply");
          heatingSaveBtn?.click();
          await waitForButtonIdle(heatingSaveBtn);
        }));
      }
      ...
      if(mixReloadBtn){
        mixReloadBtn.addEventListener("click", () => withButtonBusy(mixReloadBtn, "Načítám…", async () => {
          const heatingReloadBtn = document.getElementById("hReloadConfig");
          heatingReloadBtn?.click();
          await waitForButtonIdle(heatingReloadBtn);
        }));
      }
      bindImmediateButton(document.getElementById("mixCalGoA"), "Přejíždím…", () => mixManualMoveToEnd("a"));
      bindImmediateButton(document.getElementById("mixCalGoB"), "Přejíždím…", () => mixManualMoveToEnd("b"));
      ...
      const mixSyncABtn = document.getElementById("hMixSyncA");
      if(mixSyncABtn) mixSyncABtn.addEventListener("click", () => withButtonBusy(mixSyncABtn, "Ukládám A…", () => mixCalibrationCommand("sync_a")));
      const mixSyncBBtn = document.getElementById("hMixSyncB");
...
const tooltipHelpById = {
  ...
  hMixEnabled: "Povolí automatické pulzní ovládání trojcestného směšovacího ventilu relé 1 a 2.",
  hMixOpenRelay: "Relé použité pro pohyb ventilu směrem A, tedy pro otevírání podle konfigurace soustavy.",
  hMixCloseRelay: "Relé použité pro pohyb ventilu směrem B, tedy pro zavírání podle konfigurace soustavy.",
  hMixDeadband: "Pásmo kolem cílové teploty, ve kterém se ventil nepohybuje. Omezuje kmitání a časté spínání relé.",
  hMixTargetOffsetC: "Zvýšení cíle pouze pro podporu topení z akumulační nádrže. OpenTherm kotli nadále odesílá základní ekvitermní cíl bez tohoto offsetu.",
  hMixTargetReachedAction: "Určuje, zda se po dosažení zvýšeného cíle podpory ventil přesune do krajní polohy A, nebo zůstane v právě dosažené poloze.",
  hMixPulseMs: "Délka jednoho regulačního impulzu motoru směšovacího ventilu v milisekundách.",
  hMixMinIntervalMs: "Doba od vypnutí relé do povolení dalšího pulsu. Stejnou dobu musí AB nepřetržitě zůstat v cílovém pásmu; změna teploty odpočet ustálení restartuje. Akce po dosažení cíle se provede až po potvrzené reakci ventilu.",
