...
const renderIO = () => {
  ...
  const ensureConfigShape = () => {
    ...
    e.minFlow = Number.isFinite(Number(e.minFlow)) ? Number(e.minFlow) : 25;
    e.maxFlow = Number.isFinite(Number(e.maxFlow)) ? Number(e.maxFlow) : 55;
    e.offsetDay = Number.isFinite(Number(e.offsetDay)) ? Number(e.offsetDay) : 0;
    e.offsetNight = Number.isFinite(Number(e.offsetNight)) ? Number(e.offsetNight) : 0;
    e.dayPoints = Array.isArray(e.dayPoints) ? e.dayPoints : [{ tout: -10, tflow: 45 }, { tout: 15, tflow: 30 }];
    e.nightPoints = Array.isArray(e.nightPoints) ? e.nightPoints : [{ tout: -10, tflow: 40 }, { tout: 15, tflow: 25 }];
    e.dayPoints = e.dayPoints.map(p => ({ tout: Number(p?.tout ?? 0), tflow: Number(p?.tflow ?? 0) }));
    e.nightPoints = e.nightPoints.map(p => ({ tout: Number(p?.tout ?? 0), tflow: Number(p?.tflow ?? 0) }));
