...
(function(){
  ...
  App.pages.thermometers = {
    ...
    async mount(root){
      ...
      root.innerHTML = `
        <div class="pageHead">
          <div class="pageTitle">Teploměry</div>
          <div class="pageActions">
            <button class="btn" data-act="load">Načíst</button>
            <button class="btn primary" data-act="save" disabled>Uložit</button>
            <button class="btn btn--ghost" data-act="refresh">Obnovit</button>
          </div>
        </div>
        <div class="grid12">
          <div class="col12">
            <div class="card">
              <div class="card__head">
                ...
                <div class="card__actions">
                  <span class="tag muted">konfigurace rolí</span>
                </div>
              </div>
    ...
    <div class="card__body">
      ...
      <div class="hint" style="margin-top:10px" data-bleHint>—</div>
          ...
          <div class="col12">
            <div class="card">
              <div class="card__head">
                <div class="card__title">Dallas mapování (T1..T8 → GPIO0..GPIO3 + volba konkrétního čidla)</div>
              </div>
              <div class="card__body" data-dallasmap></div>
              <div class="card__foot">
                <div class="muted">Tip: Pokud máš na jednom GPIO více DS18B20, nastav u slotu konkrétní ROM (jinak AUTO vezme „první validní“).</div>
              </div>
            </div>
          </div>
      ...
      this._els.dallasMap = root.querySelector('[data-dallasmap]');
...
_renderRows(){
    ...
    _renderDallasMap(){
      const host = this._els.dallasMap;
      if (!host) return;
      const cfg = (App.state && App.state.config && typeof App.state.config === 'object') ? App.state.config : {};

      const gpios = Array.isArray(cfg.dallasGpios) ? cfg.dallasGpios : Array.from({length:8}).map((_,i)=> (i<=3?i:-1));
      const addrs = Array.isArray(cfg.dallasAddrs) ? cfg.dallasAddrs : Array.from({length:8}).map(()=>"");

      // Build per-gpio ROM list from /api/dallas diagnostics
      const byGpio = {};
      const gArr = this._dallas?.gpios || [];
      for (const g of gArr) {
        const gg = Number(g.gpio);
        const devs = Array.isArray(g.devices) ? g.devices : [];
        byGpio[gg] = devs.map(d=>String(d.romHex||'')).filter(x=>x);
      }

      const grid = document.createElement('div');
      grid.className = 'grid2';

      const makeGpioSel = (val)=>{
        ...
        for (const o of opts){
          ...
          op.value=String(o.v);
          op.textContent=o.t;
          if (Number(val)===Number(o.v)) op.selected=true;
          s.appendChild(op);
        }
        return s;
      };

      const makeRomSel = (gpio, val)=>{
        const s = document.createElement('select');
        const o0=document.createElement('option');
        o0.value='';
        o0.textContent='AUTO (první validní)';
        if (!val) o0.selected=true;
        s.appendChild(o0);
        const list = byGpio[Number(gpio)] || [];
        for (const rom of list){
          const op=document.createElement('option');
          op.value=rom;
          op.textContent=rom;
          if (String(val).toUpperCase()===String(rom).toUpperCase()) op.selected=true;
          s.appendChild(op);
        }
        return s;
      };
      ...
      for (let i=0;i<8;i++){
        ...
        const selG = makeGpioSel(gpios[i] ?? (i<=3?i:-1));
        const selR = makeRomSel(selG.value, addrs[i] || '');

        selG.onchange = ()=>{
          // rebuild ROM select on gpio change
          const newSel = makeRomSel(selG.value, '');
          newSel.onchange = ()=>{};
          wrap.replaceChild(newSel, selR);
          rows[i].selR = newSel;
        };
      ...
      btn.className='btn';
      btn.textContent='Uložit mapování Dallas';
      ...
      btn.onclick = async ()=>{
        ...
        try{
          ...
          obj.dallasGpios = rows.map(r=> Number(r.selG.value));
          obj.dallasAddrs = rows.map(r=> String(r.selR.value||''));
          await App.api.postText('/api/config/apply', JSON.stringify(obj, null, 2), 'application/json');
          // reload to reflect stored config + slot ROMs
          await App.roles.loadConfig();
          await this._loadDallas();
          this._renderDallasDiag();
