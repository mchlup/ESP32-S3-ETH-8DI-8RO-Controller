...
(() => {
  ...
  // Trojcestný ventil 2‑bod: master=DOi, peer vždy DO(i+1). Peer je pevně nastavený a UI ho zamkne.
  const reconcile3WayPairs = (cfg) => {
    ...
    for (let i = 0; i < outs.length; i++) {
      let o = outs[i];
      if (o.role === "valve_3way_spring") o.role = "valve_3way_2rel"; // legacy migrate
    }
    ...
    for (let i = 0; i < outs.length; i++) {
      ...
      // last relay cannot be master
      if (i === outs.length - 1) {
        o.role = "none";
        o.params = {};
        toast?.("DO8 nemůže být master trojcestného ventilu (potřebuje DO8+1).");
        continue;
      }

      o.params = (o.params && typeof o.params === "object") ? o.params : {};
      const partnerRelay1based = i + 2; // DO(i+2)
      o.params.partnerRelay = partnerRelay1based;

      // If partner is master, degrade it
      if (outs[i + 1].role === "valve_3way_2rel") {
        outs[i + 1].role = "none";
        outs[i + 1].params = {};
      }

      outs[i + 1].role = "valve_3way_peer";
      outs[i + 1].params = { master: i + 1 };
    }
  };
      ...
      return `
        <div class="inline" style="flex-wrap:wrap;gap:10px">
          <div class="muted" style="min-width:260px;align-self:flex-end">
            Použije DO${idx+1} (směr A) + <b>DO${partner}</b> (směr B). DO${partner} je automaticky uzamčeno.
          </div>
  ...
  const bind = () => {
    ...
    const onParamInput = (ev) => {
      ...
      // keep partnerRelay deterministic for 3way master
      if (typ === "out" && container.role === "valve_3way_2rel") {
        container.params.partnerRelay = idx + 2;
      }
    };
