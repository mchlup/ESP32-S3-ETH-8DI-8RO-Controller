...
extern "C" void openThermBackgroundService(void) __attribute__((weak));
#if !defined(__AVR__)
...
{
    // DataLink Layer frame format (protocol v2.2, section 4.2):
    // P (bit31), MSG-TYPE (bits30..28), SPARE (bits27..24 = 0), DATA-ID (bits23..16), DATA-VALUE (bits15..0)
    unsigned long request = (unsigned long)(data & 0xFFFF);
    request |= ((unsigned long)type) << 28;
    request |= ((unsigned long)id) << 16;
    // Set parity bit so that total number of '1' bits in the 32-bit frame is even (protocol v2.2, section 4.2.1)
    if (parity(request))
        request |= (1ul << 31);
    return request;
}
...
bool OpenTherm::isValidRequest(unsigned long request)
{
    ...
    return msgType == (byte)OpenThermMessageType::READ_DATA || msgType == (byte)OpenThermMessageType::WRITE_DATA || msgType == (byte)OpenThermMessageType::INVALID_DATA;
}
